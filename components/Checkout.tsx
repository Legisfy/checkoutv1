    ...
    try {
      const response = await fetch('https://wvvxstgpjodmfxpekhkf.supabase.co/functions/v1/create-billing', {
        ...
        body: JSON.stringify({ 
          ...
          trial_days: 7 // Garantindo que todos os planos tenham 7 dias gratuitos
        }),
      });
      const data = await response.json();
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-stretch">
        ...
        <div className="w-full lg:w-[380px] flex flex-col gap-4">
          ...
          <div className={`flex-1 bg-gradient-to-b ${selectedPlan ? planColorMap[getPlanKey(selectedPlan.name)] || 'from-zinc-800 to-zinc-900 border-zinc-700' : 'from-zinc-800 to-zinc-900 border-zinc-700'} border backdrop-blur-xl rounded-[2rem] p-8 shadow-2xl shadow-black/50 relative overflow-hidden flex flex-col justify-between group/card`}>
            ...
            {/* Banner de 7 Dias Gratuitos */}
            <div className="mt-8 mb-4 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl animate-in fade-in slide-in-from-top-4 duration-700">
              <div className="flex items-center gap-3 mb-1">
                <div className="p-1.5 bg-emerald-500/20 rounded-lg">
                  <Rocket size={14} className="text-emerald-400" />
                </div>
                <span className="text-[10px] font-bold text-emerald-300 uppercase tracking-widest">Oferta de Lançamento</span>
              </div>
              <h3 className="text-sm font-bold text-white mb-1">07 Dias Gratuitos</h3>
              <p className="text-[9px] text-white/50 leading-relaxed">
                Aproveite acesso total por 7 dias. Você pode cancelar a qualquer momento sem custos. A cobrança só será efetuada após o período de teste.
              </p>
            </div>
            ...
            <div className="space-y-5">
              <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 relative overflow-hidden shadow-lg">
                <div className="flex justify-between items-center mb-2 relative z-10">
                  ...
                  <span className="text-white/50 font-bold text-sm tracking-tight line-through opacity-50">
                    R$ {selectedPlan ? selectedPlan.price.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) : '0,00'}
                  </span>
                </div>
                <div className="flex justify-between items-center mb-4 relative z-10">
                  <span className="text-emerald-400 text-[8px] font-bold uppercase tracking-[0.2em]">Devido Hoje</span>
                ...
                <div className="flex justify-between items-center pt-4 border-t border-white/10 relative z-10">
                  <div className="flex flex-col">
                    <span className="text-white/30 text-[7px] font-semibold uppercase tracking-[0.2em]">Próxima Cobrança</span>
                    <span className="text-white/20 text-[6px] font-medium uppercase mt-0.5">Em exatamente 7 dias</span>
                  </div>
                  <span className="text-white/60 font-bold text-[9px] tracking-widest">R$ {selectedPlan ? selectedPlan.price.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) : '0,00'}</span>
                </div>
              </div>
            </div>
          </div>
