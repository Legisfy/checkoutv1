  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8 md:py-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      <div className="text-center mb-10">
        ...
        <h1 className="text-4xl md:text-6xl font-black mb-3 tracking-tighter leading-none text-white italic">
          Sucesso <span className="text-slate-500 font-medium not-italic">Confirmado</span>
        </h1>
        <p className="text-slate-400 text-base max-w-2xl mx-auto leading-relaxed font-medium opacity-80">
          Prezado(a) <span className="text-white font-black">{orderData.fullName?.split(' ')[0]}</span>, seu mandato acaba de subir de nível com a <span className="text-white font-black tracking-widest uppercase text-sm ml-1">Legisfy</span>.
        </p>
      </div>
        ...
        <div className="lg:col-span-6 bg-[#0d1017] border border-white/5 rounded-[2.5rem] p-8 md:p-10 backdrop-blur-2xl shadow-xl ring-1 ring-white/5 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-lg font-black flex items-center gap-3 text-white uppercase tracking-tighter italic">
                <Mail size={20} className="text-slate-400" />
                Confirmação
              </h3>
              <span className="text-[9px] font-black uppercase tracking-[0.25em] text-slate-500 bg-slate-900/80 px-4 py-2 rounded-full border border-white/5">#LG-98412</span>
            </div>
            
            <div className="space-y-6 mb-8">
              <div className="flex justify-between items-center group">
                <span className="text-slate-600 font-black text-[10px] uppercase tracking-widest">SISTEMA</span>
                <span className="text-white font-black text-lg group-hover:tracking-wider transition-all duration-500 uppercase tracking-tighter italic">LEG-BUSINESS</span>
              </div>
              
              <div className="pt-6 border-t border-white/5 flex justify-between items-end">
                <span className="text-slate-600 font-black text-[10px] uppercase tracking-widest mb-1">VALOR</span>
                <div className="flex flex-col items-end">
                  <span className="text-4xl md:text-5xl font-black text-white tracking-tighter italic">R$ 497,00</span>
                  <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest mt-1 opacity-50">MENSALIDADE</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 bg-white/5 rounded-2xl border border-white/5 flex items-center gap-4">
            ...
            <div>
              <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest mb-0.5">LOCALIDADE</p>
              <p className="text-sm text-slate-200 font-black tracking-tight">
          ...
          {isPix ? (
            <div className="flex flex-col items-center w-full relative z-10 text-black">
              <div className="text-center mb-6">
                <h3 className="text-2xl font-black mb-1 uppercase tracking-tighter italic">Pagamento PIX</h3>
              ...
              <div className="w-full space-y-4">
                <button
                  onClick={handleCopy}
                  className={`w-full py-5 rounded-xl font-black text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 transition-all transform active:scale-95 shadow-lg ${
                    copied 
            ...
            <div className="flex flex-col justify-between h-full w-full relative z-10 text-black">
              <div className="mb-10">
                ...
                <h3 className="text-3xl font-black mb-4 leading-tight tracking-tighter italic">Gabinete Digital<br/>Liberado.</h3>
                <p className="text-base text-slate-600 font-medium leading-relaxed opacity-90">
                  Credenciais enviadas para <span className="text-black font-black underline decoration-black/20">{orderData.email}</span>.
                </p>
              </div>
              
              <button className="w-full py-6 bg-black text-white font-black text-base rounded-2xl transition-all flex items-center justify-center gap-3 hover:tracking-widest shadow-xl active:scale-95 group">
                ENTRAR NO PAINEL
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-10 pt-10 border-t border-white/5">
        <button className="flex items-center gap-3 text-slate-600 hover:text-white transition-all text-[10px] font-black uppercase tracking-[0.2em] group">
          <Printer size={16} />
          Recibo Fiscal
        </button>
        <button className="flex items-center gap-3 text-slate-600 hover:text-white transition-all text-[10px] font-black uppercase tracking-[0.2em] group">
